...
def run_models():
    ...
    for model in models_name:
        ...
        cmd = f'cd {model}; python train.py --steps {model_steps[model]} --data_location={model_path}/data'
        print(cmd)
        os.system(cmd)
        ...
        if model_auc < model_standard_auc[model]:
            ...
            model_scores[model+'_failed'] = str(1)
        model_total_speed += model_speed
        print(f'{model}, speed: {model_speed}, auc: {model_auc}, is_valid: {model_auc >= model_standard_auc[model]}')
